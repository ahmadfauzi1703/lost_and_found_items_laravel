...
<head>
  ...
  <title>Items Lost - Sipanang </title>
...
<body class="bg-gray-100">
  {{-- Halaman admin untuk meninjau dan memfilter laporan barang hilang --}}
  <!-- Sidebar -->
  <div class="flex h-screen">
    <div class="w-[15rem] flex-shrink-0 bg-[#124076] text-white z-[1000px]">
      <img
      ...
      <ul class="mt-6 space-y-2">
        ...
        <li>
          <a href="{{ route('admin_dashboard_lost') }}" class="block px-4 py-2 bg-[#1E5CB8]"><i class='bx bxs-box'></i> Barang Hilang</a>
        </li>
        <li>
    ...
    <div class="flex-1 p-3 overflow-hidden">
      <div class="bg-white rounded-lg shadow-lg p-6">
        ...
        <div class="flex justify-between mb-4">
          {{-- Filter kategori/pencarian untuk menemukan laporan tertentu --}}
          <form method="GET" action="{{ route('admin_dashboard_lost') }}" class="flex space-x-4">
            ...
            <a href="{{ route('admin_dashboard_lost') }}" class="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600">Clear</a>
            @endif
          </form>
          <!-- Filter Info Section -->
          <div class="mb-4">
            ...
            <p class="text-sm text-gray-600">
              ...
              ({{ $lostItems->count() }} item ditemukan)
            </p>
            @else
            <p class="text-sm text-gray-600">Menampilkan semua item: {{ $lostItems->count() }} item</p>
            @endif
          </div>
        </div>
          ...
          <div class="-m-1.5 overflow-x-auto">
            <div class="p-1.5 min-w-full inline-block align-middle">
              <div class="overflow-hidden">
                {{-- Daftar laporan hilang beserta aksi hapus --}}
    ...
    {{ $item->status == 'approved' ? 'bg-green-100 text-green-800' : 
      ...
      'bg-red-100 text-red-800') }}">
                      ...
                      <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                        <!-- <button onclick="openEditModal('{{ $item->id }}')" class="inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent text-blue-600 hover:text-blue-800 focus:outline-hidden focus:text-blue-800">
                          Edit
                        </button> -->
                    ...
                    <tr class="bg-white">
                      <td colspan="9" class="px-6 py-4 text-sm text-gray-800 text-center">No items found</td>
                    </tr>
                    @endif
                  </tbody>
                </table>
  ...
  <script>
    function openEditModal(id) {
      // You can implement AJAX to fetch the item details
